...
# -------------------------
# Column integrals in numpy: cwp/rw using trapezoid in logp
# -------------------------
def cumtrapz_np(y, x):
    """
    y,x: (B,N)
    returns integral from 0..i with trapezoid, same shape, out[:,0]=0
    """
    B, N = y.shape
...
def cwp_rw_from_q_logp_np(q, logp):
    """
    q,logp: (B,N)
    cwp(i)=∫_{top→i} q dlogp  (cumulative from top)
    rw(i) =∫_{i→surf} q dlogp  (remaining to surface)
    """
    cwp = cumtrapz_np(q, logp)
    total = cwp[:, -1:]  # (B,1)
    rw = total - cwp
    return cwp.astype(np.float32), rw.astype(np.float32)
...
def recon_flux_from_hr_toa_boa(
    ...
    delta is distributed ONLY in bottom window (last bot_window_k levels).
    """
    # integrate HR over logp
    f_tilde = cumtrapz_torch(HR, logp)               # (B,N)
    f1 = f_tilde + TOA[:, None]                      # anchor TOA
    delta = (BOA - f1[:, -1])                        # (B,)
...
def main():
    ...
    ap.add_argument("--outdir", type=str, default="stepB2p3c_out")
    ...
    print("Added features: cwp/rw (∫ q dlogp)")
    ...
    # ---- compute cwp/rw on native grid for train stats + val features
    cwp_all, rw_all = cwp_rw_from_q_logp_np(q, logp)  # (S,N0)

    # ---- fit feature normalization on TRAIN at native grid (now 6-dim)
    Ts_b_tr = np.repeat(Ts[tr_idx, None], N0, axis=1).astype(np.float32)
    Xtr = np.stack(
        [T[tr_idx], logp[tr_idx], q[tr_idx], Ts_b_tr, cwp_all[tr_idx], rw_all[tr_idx]],
        axis=-1
    ).astype(np.float32)  # (S_tr,N0,6)

    X_mu, X_std = zfit(Xtr.reshape(-1, 6))
    ...
    Xva = np.stack(
        [T[va_idx], logp[va_idx], q[va_idx], Ts_b_va, cwp_all[va_idx], rw_all[va_idx]],
        axis=-1
    ).astype(np.float32)
    ...
    # ---- model / optim
    model = GNO_HR_TOA_BOA(in_dim=6, hidden=args.hidden, K=args.K, L=args.L).to(device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for st in range(n_steps):
            ...
            # compute cwp/rw on the REGRIDDED grid (important)
            cwpn, rwn = cwp_rw_from_q_logp_np(qn, logpn)

            # features (6-dim)
            X = np.stack([Tn, logpn, qn, Tsbn, cwpn, rwn], axis=-1).astype(np.float32)
        ...
        if rmse_prof < best:
            ...
            torch.save({
                ...
                "cfg": vars(args),
                "features": ["T", "logp", "q", "Ts_broadcast", "cwp", "rw"],
                "F_mu": F_mu, "F_std": F_std,
                "H_mu": H_mu, "H_std": H_std,
                "X_mu": X_mu.squeeze(0).tolist(),
                "X_std": X_std.squeeze(0).tolist(),
                "target": "Route-B2 recon (HR+TOA+BOA -> Fnet) + cwp/rw",
            }, best_path)
