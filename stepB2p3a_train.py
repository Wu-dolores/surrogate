...
def zinvert_torch(xn, mu, std):
    # mu,std are floats or tensors broadcastable
...
def regrid_1d_profile(x, c_old, c_new):
    """
    x: (N_old,) float32
    c_old: (N_old,) float32 monotone
    c_new: (N_new,) float32
    """
    # numpy interp expects 1D
...
def regrid_batch(T, logp, q, Fnet, Ts, N_new):
    ...
    Inputs are numpy arrays:
      ...
      Ts: (B,)
    Returns regridded arrays (numpy float32):
      Tn, logpn, qn, Fnetn, Tsb
    """
    B, N_old = Fnet.shape
    # build c_tilde per sample from logp
    c_top = logp[:, 0:1]
    c_sfc = logp[:, -1:]
    c_old = (logp - c_top) / (c_sfc - c_top + 1e-6)  # (B,N_old)

    c_new = np.linspace(0.0, 1.0, N_new, dtype=np.float32)  # (N_new,)
    ...
    Tsb = np.repeat(Ts[:, None], N_new, axis=1).astype(np.float32)
    return Tn, logpn, qn, Fnetn, Tsb

# -------------------------
# Torch recon: integrate HR over logp -> flux shape, then enforce TOA/BOA with alpha blend
# -------------------------
...
def recon_flux_from_hr_toa_boa(HR, logp, TOA, BOA, alpha_gamma=1.0):
    """
    HR:  (B,N) physical
    logp:(B,N) physical
    TOA:(B,) physical
    ...
    """
    # integrate: f_tilde[0]=0
    f_tilde = cumtrapz_torch(HR, logp)               # (B,N)

    # anchor TOA at top
    f1 = f_tilde + TOA[:, None]                      # (B,N)

    # alpha in [0,1] from top->bottom
    denom = (logp[:, -1:] - logp[:, 0:1] + 1e-6)
    alpha = (logp - logp[:, 0:1]) / denom
    alpha = torch.clamp(alpha, 0.0, 1.0) ** float(alpha_gamma)

    # enforce BOA by distributing delta
    delta = (BOA - f1[:, -1])                        # (B,)
...
class GNO_HR_TOA_BOA(nn.Module):
    """
    Outputs normalized predictions:
      hr_n:  (B,N,1)
      toa_n: (B,1)
      boa_n: (B,1)
    """
...
# -------------------------
# Eval (physical RMSE on recon flux)
# -------------------------
@torch.no_grad()
def eval_val(model, Xn, logp, F_true, F_mu, F_std, H_mu, H_std, alpha_gamma, device, batch=256):
    model.eval()
    S, N, Fin = Xn.shape
    ...
    for i in range(0, S, batch):
        ...
        Ftb = torch.tensor(F_true[i:i+batch], dtype=torch.float32, device=device)
        ...
        Frec = recon_flux_from_hr_toa_boa(hr, cb, toa, boa, alpha_gamma=alpha_gamma)
...
def main():
    ...
    ap.add_argument("--outdir", type=str, default="stepB2p3a_out")
    ...
    # fit normalization on TRAIN at native grid (N0)
    ...
    # flux stats (for toa/boa and recon-space)
    F_mu = float(Fnet[tr_idx].reshape(-1).mean())
    F_std = float(Fnet[tr_idx].reshape(-1).std() + 1e-6)

    # HR stats: derive HR_true from Fnet on train native grid
    with torch.no_grad():
    ...
    # prep val arrays (native grid only)
    ...
    # loss function
    mse = nn.MSELoss(reduction="none")  # we will weight per-sample

    # ---- training loop
    # build oversample weights once (by duplicating indices)
    tail_mask = (Ts[tr_idx] >= float(args.Ts_tail))
    tail_ids = tr_idx[tail_mask]
    base_ids = tr_idx

    # if tail_mult=1 => no duplication
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for st in range(n_steps):
            ...
            # choose a random N for this step (batch-fixed N)
            ...
            X = np.stack([Tn, logpn, qn, Tsbn], axis=-1).astype(np.float32)  # (B,N,4)
            ...
            # derive HR_true from TRUE flux on this grid
            HRtrue = diffop_torch(Ftrue, coord)  # (B,N)

            # forward
            hr_n, toa_n, boa_n = model(xb, coord)

            # invert outputs to physical
            HR = zinvert_torch(hr_n.squeeze(-1), H_mu, H_std)   # (B,N)
            TOA = zinvert_torch(toa_n.squeeze(-1), F_mu, F_std) # (B,)
            BOA = zinvert_torch(boa_n.squeeze(-1), F_mu, F_std) # (B,)

            # recon flux
            Frec = recon_flux_from_hr_toa_boa(HR, coord, TOA, BOA, alpha_gamma=args.alpha_gamma)

            # losses (physical space)
            # HR loss
            Lhr = (HR - HRtrue) ** 2
            Lhr = Lhr.mean(dim=1)  # per-sample

            # endpoint losses (use recon endpoints to be consistent)
            Ltoa = (Frec[:, 0] - Ftrue[:, 0]) ** 2
            Lboa = (Frec[:, -1] - Ftrue[:, -1]) ** 2

            # full recon profile loss
            Lfrec = (Frec - Ftrue) ** 2
            Lfrec = Lfrec.mean(dim=1)  # per-sample

            # near-surface BC loss (bottom m0 layers)
            m0 = int(min(args.m0_bc, N_new))
            if m0 > 0:
                Lbc = (Frec[:, -m0:] - Ftrue[:, -m0:]) ** 2
                Lbc = Lbc.mean(dim=1)
            else:
                Lbc = torch.zeros_like(Lfrec)

            # NEW: bottom-band loss (bottom bot_k layers)
            kb = int(min(args.bot_k, N_new))
            if kb > 0:
                Lbot = (Frec[:, -kb:] - Ftrue[:, -kb:]) ** 2
                Lbot = Lbot.mean(dim=1)
            else:
                Lbot = torch.zeros_like(Lfrec)

            # sample weights: Ts-tail oversampling already duplicated indices,
            # but keep weights =1 here (simple).
            loss_per = (
                args.lam_hr * Lhr +
                args.lam_toa * Ltoa +
                args.lam_boa * Lboa +
        ...
        # ---- eval on val (native grid)
        rmse_prof, rmse_toa, rmse_boa = eval_val(
            model, Xva_n, logp_va, F_va,
            F_mu=F_mu, F_std=F_std, H_mu=H_mu, H_std=H_std,
            alpha_gamma=args.alpha_gamma, device=device, batch=256
        )
        ...
        if rmse_prof < best:
            ...
            torch.save({
                ...
                "target": "Route-B2 recon (HR+TOA+BOA -> Fnet)",
            }, best_path)
