...
def _interp1d_batch(x_old, y_old, x_new):
    ...
    x_old: (B,Nold) monotonic
    y_old: (B,Nold) or (B,Nold,C)
    x_new: (B,Nnew) monotonic (need not match x_old)
    """
...
def main():
    ...
    ap.add_argument("--data", type=str, required=True, help="train data at Nbase (e.g., N=60)")
    ap.add_argument("--template160", type=str, required=True, help="npz with real N=160 logp_arr (e.g., test_N160_1000.npz)")
    ap.add_argument("--n_templates", type=int, default=64, help="how many logp templates to subsample from template160")
    ...
    ap.add_argument("--outdir", type=str, default="step4_v1_regrid_template160_out")
    ...
    # ---- load templates (real N=160 logp grids) ----
    dt = np.load(args.template160, allow_pickle=True)
    logp160 = dt["logp_arr"].astype(np.float32)  # (S160,160)
    S160 = logp160.shape[0]
    nT = min(int(args.n_templates), S160)
    pickT = rng.choice(S160, size=nT, replace=False)
    templates160 = torch.tensor(logp160[pickT], dtype=torch.float32)  # (nT,160)
    print(f"Loaded template160: {args.template160}  | using {nT} templates")

    # ---- load train data (Nbase) ----
    d = np.load(args.data, allow_pickle=True)
    Ts   = d["Ts_K"].astype(np.float32)         # (S,)
    logp = d["logp_arr"].astype(np.float32)     # (S,Nbase)
    ...
    # normalized base val for scheduler stability
    ...
    print(f"BOA band: lam_bc={args.lam_bc}, m0_bc={args.m0_bc} at N0_ref={args.N0_ref}")
    ...
    best = float("inf")
    best_path = os.path.join(outdir, "best_v1_regrid_template160.pt")
    ...
    templates160 = templates160.to(device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for _ in range(steps):
            ...
            # ---- key: for Nnew=160, use real template grid (per-sample) ----
            if Nnew == 160:
                tidx = torch.randint(low=0, high=templates160.shape[0], size=(x_phys.shape[0],), device=device)
                c_new = templates160[tidx]  # (B,160) real grids
            else:
                c_new = make_linear_coord_from_old(c_old, Nnew)

            # unpack features: [T, logp, q, Ts]
            T_old  = x_phys[..., 0]
            q_old  = x_phys[..., 2]
            Ts_old = x_phys[..., 3]

            # interpolate to c_new
            ...
            X_new_phys = torch.stack([T_new, c_new, q_new, Ts_new], dim=-1)
        ...
        if rmse_prof < best:
            ...
            torch.save({
                ...
                "target": "Fnet",
                "train_aug": {"type": "template160", "n_templates": nT, "N_choices": N_choices},
    ...
    print("  python eval_real_testset.py --ckpt <ckpt> --data test_N160_1000.npz --out ... --diag")
    print("  python eval_real_testset.py --ckpt <ckpt> --data test_N40_1000.npz  --out ... --diag")
