...
class HR_TOA_BOA_Model(nn.Module):
    def __init__(self, in_dim=4, hidden=128, K=6, L=4, ts_idx=3):
        super().__init__()
        self.ts_idx = ts_idx
        ...
        # Modified BOA head with Ts Skip Connection
    ...
    def forward(self, x, coord):
        ...
        f_boa = self.boa_head(torch.cat([g, h_boa, ts_skip], dim=-1)) # (B,1)
...
def predict_recon(model, Xn, logp, Ftoa_mu, Ftoa_std, Fboa_mu, Fboa_std, H_mu, H_std,
    ...
    for i in range(0, S, batch):
        ...
        # Forward Euler Integration for Reconstruction
        # Matches training loss consistency
...
def main():
    ...
    ts_idx = int(ckpt.get("ts_idx", 3))
    print(f"Loaded Ts index: {ts_idx}")
    ...
    model = HR_TOA_BOA_Model(in_dim=in_dim, hidden=hidden, K=K, L=L, ts_idx=ts_idx).to(device)
    ...
    # Z-apply with broadcasted means
    ...
    print("\n=== REAL testset eval (Route-B2 Recon with Ts Skip Connection) ===")
